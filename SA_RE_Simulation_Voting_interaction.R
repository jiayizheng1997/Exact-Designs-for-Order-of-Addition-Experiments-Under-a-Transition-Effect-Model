...
comparison <- function(m=6,inter=2,vote_num=50,tot_iter=100){
  ...
  for (i in 1:length(nlist)) {
    for (iter in 1:tot_iter) { 
      ...
      n = nlist[i]
      #optFederov
      out=retry(optFederov(data=Xf[,-1],nTrials=n,criterion="I", nRepeats = 5),when = 'Singular design')
      D_out = Df[out$rows,]
      X_out <- design_to_PWO(D_out)
      for (i in 1:inter) {
        X_out = cbind(X_out,X_out[,which(colnames(X_out)==name[i])]*X_out[,which(colnames(X_out)==name[i+1])])
      }
      I_out <- I_efficiency(X_out, Bf)
      I_list_Federov[iter,i]=I_full/I_out
      #random exchange
      D_out_RE = random_exchange_v2(n=n, Df=Df, B=Bf, inter=inter,max_iter=1e4)
      ...
      #simulated annealing
      D_out_SA = simulated_annealing_v2(n=n, D=Df, B=Bf, inter=inter,max_iter=1e4)
...
out_list <- foreach(iter = 1:nsim, .packages = c("combinat", "matrixcalc", "reshape2", "AlgDesign",
    ...
    m = DF[iter,2]
    i = DF[iter,1]
    comparison_list <- comparison(m=m,inter=i,vote_num=vote_num,tot_iter=100)
...
# comparison_list1 <- comparison(m=6,inter=2,vote_num=50,tot_iter=100)
