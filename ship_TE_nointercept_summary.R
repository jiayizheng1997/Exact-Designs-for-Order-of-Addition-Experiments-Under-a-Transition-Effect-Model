...
# source("TE_PWO_CP_FP_modelsource_nointercept.R") # gives the same output
...
# cons_BB_PWO_model = lm(Y_out_BB ~-1+., data = esc_BB)
...
# cons_GRASP_PWO_model = lm(Y_out_GRASP ~-1+., data = esc_GRASP)
...
# cons_BB_CP_model = lm(Y_out_BB ~-1+., data = esc_BB)
...
# cons_GRASP_CP_model = lm(Y_out_GRASP ~-1+., data = esc_GRASP)
...
# cons_BB_FP_model = lm(Y_out_BB ~-1+., data = esc_BB)
...
# cons_GRASP_FP_model = lm(Y_out_GRASP ~-1+., data = esc_GRASP)
...
##########################################
### Here starts our needed data ###
##########################################


c(AIC(cons_BB_PWO_model),AIC(cons_BB_TE_model),AIC(cons_BB_TE2_model),AIC(cons_BB_CP_model),AIC(cons_BB_FP_model))
c(AIC(cons_GRASP_PWO_model),AIC(cons_GRASP_TE_model),AIC(cons_GRASP_TE2_model),AIC(cons_GRASP_CP_model),AIC(cons_GRASP_FP_model))


#c(BIC(cons_BB_PWO_model),BIC(cons_BB_TE_model),BIC(cons_BB_TE2_model),BIC(cons_BB_CP_model),BIC(cons_BB_FP_model))
#c(BIC(cons_GRASP_PWO_model),BIC(cons_GRASP_TE_model),BIC(cons_GRASP_TE2_model),BIC(cons_GRASP_CP_model),BIC(cons_GRASP_FP_model))

c(mean((Y-BB_PWO_pred)^2),mean((Y-BB_TE_pred)^2),mean((Y-BB_TE2_pred)^2),mean((Y-BB_CP_pred)^2),mean((Y-BB_FP_pred)^2))
c(mean((Y-GRASP_PWO_pred)^2),mean((Y-GRASP_TE_pred)^2),mean((Y-GRASP_TE2_pred)^2),mean((Y-GRASP_CP_pred)^2),mean((Y-GRASP_FP_pred)^2))

# summary(cons_BB_TE_model)
# summary(cons_GRASP_TE_model)

tmat_BB = matrix(NA, nrow = 6, ncol = 6)
tmat_GRASP = matrix(NA, nrow = 6, ncol = 6)
ind = 1
for(i in 1:6){
  for(j in 1:6){
    if(i != j){
      tmat_BB[i,j] = round(coef(cons_BB_TE_model)[ind],2)
      tmat_GRASP[i,j] = round(coef(cons_GRASP_TE_model)[ind],2)
      ind = ind + 1
    }
  }
}
