...
e = 1
set.seed(1234)
block <- matrix(c(1,2,5,6,0,0),nrow=2,byrow = TRUE) #Generate block matrices
Df <- generate_full_design(m) #Calculate the nlist
...
while (iter<=100){
  ...
  skip_to_next <- FALSE
  tryCatch(D_out_RE<-random_exchange(D_start,Bf,block=block),error=function(e){skip_to_next<<-TRUE})
  if(skip_to_next) { next }   
  X_out_RE <- design_to_PWO(D_out_RE)
  X_out_RE <- X_out_RE[,-filter$cons_cols]
  skip_to_next <- FALSE
  tryCatch(D_out_SA <- simulated_annealing(D_start, Bf, block = block),error=function(e){skip_to_next<<-TRUE})
  if(skip_to_next) { next } 
...
mu = 3
e = 0
block <- matrix(c(1,2,4,6),nrow=2,byrow = TRUE) #Generate block matrices
Df <- generate_full_design(m) #Calculate the nlist
n_sim <- 100
rank_RE <- rep(NA,n_sim)
rank_SA <- rep(NA,n_sim)
