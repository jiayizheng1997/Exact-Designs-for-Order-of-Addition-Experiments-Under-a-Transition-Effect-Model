...
RE_voting <- function(n,Df, vote_n, vote_num, block=block, filter, Bf, max_iter=1e4){
  ...
  for (voter in 1:vote_num) {
    ...
    for (i in 1:nrow(D_out_RE)){
      vote_RE[i,voter] = which(apply(Df, 1, function(x) identical(D_out_RE[i,],x)))
    }
...
SA_voting <- function(n,Df, vote_n, vote_num, block=block, filter, Bf, max_iter=1e4){
  ...
  for (voter in 1:vote_num) {
    ...
    for (i in 1:nrow(D_out_SA)){
      vote_SA[i,voter] = which(apply(Df, 1, function(x) identical(D_out_SA[i,],x)))
    }
...
RE_voting_v2 <- function(n,Df, vote_n, vote_num, block, filter, Bf, max_iter=1e4){
  ...
  for (voter in 1:vote_num) {
    ...
    skip_to_next <- FALSE
    # D_out_RE<-random_exchange_v2(n, Df,Bf,block=block, filter = filter,max_iter)
    tryCatch(D_out_RE<-random_exchange_v2(n, Df,Bf,block=block, filter = filter,max_iter),error=function(e){skip_to_next<<-TRUE})
    if(skip_to_next) { next }   
    
    candidate_matrix = rbind(candidate_matrix, D_out_RE) # add the new candidates to the list
    candidate_matrix = unique(candidate_matrix) # filter out duplicate rows and preserve the order
    print(candidate_matrix)
    
    for (i in 1:nrow(D_out_RE)){
      vote_RE[i,voter] = which(apply(candidate_matrix, 1, function(x) all(identical(D_out_RE[i,],x)) ))
      # ok, all() is actually not needed above, maybe double check
  ...
  D_out_RE <- candidate_matrix[t(as.numeric(names(sort(table(vote_RE),decreasing=TRUE)[1:n]))),]
...
# v2 update: Instead of using the full design and calling identical() to count votes,
# we create a set of unique rows that is updated as more "candidates" for "votes" come in
SA_voting_v2 <- function(n,Df, vote_n, vote_num, block, filter, Bf, max_iter=1e4){
  ...
  for (voter in 1:vote_num) {
    ...
    candidate_matrix = rbind(candidate_matrix, D_out_SA) # add the new candidates to the list
    candidate_matrix = unique(candidate_matrix) # filter out duplicate rows and preserve the order
    
    for (i in 1:nrow(D_out_SA)){
      vote_SA[i,voter] = which(apply(candidate_matrix, 1, function(x) all(identical(D_out_SA[i,],x)) ))
      # ok, all() is actually not needed above, maybe double check
  ...
  D_out_SA <- candidate_matrix[t(as.numeric(names(sort(table(vote_SA),decreasing=TRUE)[1:n]))),]
...
# v2 update: Instead of using the full design and calling identical() to count votes,
# we create a set of unique rows that is updated as more "candidates" for "votes" come in
RE_voting_v3 <- function(n,Df, vote_n, vote_num, block, filter, Bf, max_iter=1e4){
...
SA_voting_v3 <- function(n,Df, vote_n, vote_num, block, filter, Bf, max_iter=1e4){
  ...
  for (voter in 1:vote_num) {
    ...
    skip_to_next <- FALSE
    tryCatch(D_out_SA<-random_exchange_v2(n, Df,Bf,block=block, filter = filter,max_iter),error=function(e){skip_to_next<<-TRUE})
    if(skip_to_next) { next }   
...
#--------------------------------------------
comparison <- function(data_row,vote_num=50,tot_iter=100){
  ...
  #Df <- Df[-filter$cons_rows,] # filter df and generate id for optfederov to initiate
  #id = matrix(apply(Df,1,function(x) as.numeric(paste(x,sep='',collapse=''))),ncol=1)
  ...
  for (i in 1:length(nlist)) {
    for (iter in 1:tot_iter) { 
      ...
      #skip_to_next <- FALSE
      #tryCatch(out = optFederov(data=Xf[,-1],nTrials=n,criterion="I"),error=function(e){skip_to_next<<-TRUE})
      #if(skip_to_next) { next }
      # out = optFederov(data=Xf[,-1],nTrials=n,criterion="I") 
      ...
      vote_n = n
      if (alg==1){
        D_out_vote <- RE_voting_v3(n = n, Df = Df, vote_n = vote_n, vote_num = vote_num, block=block, filter = filter, Bf = Bf,max_iter = 50)
      }
      else {
        D_out_vote <- SA_voting_v3(n = n, Df = Df, vote_n = vote_n, vote_num = vote_num, block=block, filter = filter, Bf = Bf,max_iter = 50)
      }
  ...
  if (alg==1){colnames(preplot)=c("optFederov","RandomExchangeVote","n")}
  else {colnames(preplot)=c("optFederov","SimulatedAnnealingVote","n")}
...
comparison_list1 <- foreach(l = 1:24,
                            .packages = c("combinat", "matrixcalc", "reshape2",
                                          "ggplot2","retry","AlgDesign")) %dopar% {
                                            comparison(DF[l,], vote_num = vote_num)
                                          }

write.csv(comparison_list1, "df1_out.csv")
print("m = 6 DONE")
...
write.csv(comparison_list2, "df2_out.csv")
print("m = 7 DONE")
...
write.csv(comparison_list3, "df3_out.csv")
print("m = 8 DONE")
...
Fedrov_id = matrix(apply(D_start,1,function(x) as.numeric(paste(x,sep='',collapse=''))),ncol=1)
rows = apply(Fedrov_id, 1, function(x) which(id==x))