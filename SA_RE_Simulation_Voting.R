...
comparison <- function(data_row,vote_num=50,tot_iter=100){
  ...
  for (i in 1:length(nlist)) {
    for (iter in 1:tot_iter) { 
      ...
      skip_to_next <- FALSE
      if (alg==1){
        tryCatch(D_out<-random_exchange_v2(n=n, Df=Df, B=Bf, block=block, filter=filter,max_iter = 2500),error=function(e){skip_to_next<<-TRUE})
      }
      if (alg==2){
        tryCatch(D_out<-simulated_annealing_v2(n=n, Df=Df, B=Bf, block=block, filter=filter,max_iter = 2500),error=function(e){skip_to_next<<-TRUE})
      }
      if(skip_to_next) { next }   
