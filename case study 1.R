...
Xf = design_to_PWO(Df)
case1 = cbind(case1,Xf)

# fit a full model without constraints
full_model = lm(Y ~ z12 + z13 + z14 + z23 + z24 + z34, data = case1)
summary(full_model)



# order pair (3,1) to be constrained as they are the most insignificant under full model
block = matrix(c(3,1),nrow = 2)
cons = cons_pair(block)

# constrain the data
filter = PWO_filter(Z=Xf,cons=cons)
Df = Df[-filter$cons_rows,]
Xf = Xf[-filter$cons_rows,-filter$cons_cols]
Y = case1$Y[-filter$cons_rows]

# from the cons model we know that the terms z23 and z34 are the most significant
# therefore, we hope we can detect the same significance in the reduced model
case1_cons = as.data.frame(cbind(Xf[,-1],Y))
cons_model = lm(Y ~ z12 + z14 + z23 + z24 + z34, data = case1_cons)
summary(cons_model)
...
while (iter<=n_sim){
  ...
  for (i in 1:n) {
    Y_out_RE[i] = Y[which(apply(Df, 1, function(x) identical(D_out_RE[i,],x)))]
  }
  X_out_RE = X_out_RE[,-1]
  case1_RE = as.data.frame(cbind(X_out_RE,Y_out_RE))
  cons_RE_model = lm(Y_out_RE ~ z23+z34+z12+z14+z24, data = case1_RE)
  ...
  # separate regressed and predicted data
  for (i in 1:n) {
    Y_out_SA[i] = Y[which(apply(Df, 1, function(x) identical(D_out_SA[i,],x)))]
  }
  X_out_SA = X_out_SA[,-1]
  case1_SA = as.data.frame(cbind(X_out_SA,Y_out_SA))
  cons_SA_model = lm(Y_out_SA ~ z23+z34+z12+z14+z24, data = case1_SA)
  ...
  cons_rand_model = lm(Y_out_rand ~ z23+z34+z12+z14+z24, data = case1_rand)
