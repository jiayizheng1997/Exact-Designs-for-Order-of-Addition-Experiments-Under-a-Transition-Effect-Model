...
comparison <- function(m=6,inter=2,vote_num=50,tot_iter=100,
  ...
  for (i in 1:length(nlist)) {
    ...
    for (iter in 1:tot_iter) {
      ...
      # I_effs = c(I_Federov, I_insertion, I_SA_vote, I_SA)
      #}
      
      I_list_Federov[iter,i] = I_Federov
      I_list_insertion_sort[iter,i] = I_insertion
      I_list_SA_vote[iter,i] = I_SA_vote
      I_list_SA[iter,i] = I_SA
    }  
...
comparison_list1 <- comparison(m=6,inter=2,vote_num=vote_num,tot_iter=2, max_iter = 10000)
