...
#This is a function to generate constrainted blocks 
#based on the extremeness of biasedness and number of free components. 
#We use it when extremeness is zero.
...
#After the generation of block， we need to convert them into pairwise 
#indications and therefore filter the constraint rows
...
#--------------------------------------------
comparison <- function(data_row,tot_iter=100){
  ...
  # nlist = round(c(nrow(Xf)/4,nrow(Xf)/2,3*nrow(Xf)/4))
  I_list_RE = matrix(NA,nrow = tot_iter, ncol = length(nlist))
  I_list_SA = matrix(NA,nrow = tot_iter, ncol = length(nlist))
  I_name <- c(paste("m=",m,",n=",nlist,"b=",b,",mu=",mu,",e=",e,sep=""))
  colnames(I_list_RE) = I_name
  colnames(I_list_SA) = I_name
  ...
  for (n in 1:length(nlist)) {
    for (iter in 1:tot_iter) {  
      ...
      skip_to_next <- FALSE
      tryCatch(D_out_RE<-random_exchange(D_start,Bf,block=block,max_iter=10),error=function(e){skip_to_next<<-TRUE})
      if(skip_to_next) { next }   
      X_out_RE <- design_to_PWO(D_out_RE)
      X_out_RE <- X_out_RE[,-filter$cons_cols]
      skip_to_next <- FALSE
      tryCatch(D_out_SA <- simulated_annealing(D_start, Bf, block = block, max_iter = 10),error=function(e){skip_to_next<<-TRUE})
      if(skip_to_next) { next } 
      ...
      I_out_RE <- I_efficiency(X_out_RE, Bf)
      I_list_RE[iter,n]=I_full/I_out_RE
      I_out_SA <- I_efficiency(X_out_SA, Bf)
      I_list_SA[iter,n]=I_full/I_out_SA
    }
  }
  preplot = cbind(matrix(I_list_RE,ncol = 1),matrix(I_list_SA,ncol = 1))
  preplot = as.data.frame(cbind(preplot,rep(nlist,each=tot_iter)))
  colnames(preplot)=c("RandomExchange","SimulationAnnealing","n")
  ...
  #the old correct version p <- ggplot(preplot_long, aes(x=n,y=value,fill=n),plot = FALSE)+
  #  geom_boxplot() + labs(title=paste("Algorithm Efficiency Comparison when m = ",m,",b=",b,",mu=",mu,",e=",e,sep = "")) + facet_wrap(~variable) +  ylab("Relative I-efficiency")
  #ggplot(preplot_long,aes(x=n,y=value,fill=n),plot=FALSE)+geom_boxplot()+labs(facet_wrap(~variable))+ylim(0.5,1)+theme(axis.title.y=element_blank())
  #+theme(legend.position="none")
  #we want to remove title and ylab and restrict y range
  #+ylab("Relative I-efficiency")
  #title=paste("Algorithm Efficiency Comparison when m = ",m,",b=",b,",mu=",mu,",e=",e,sep = "") +
  png(paste("m",m,"b",b,"mu",mu,"e",e,".png",sep = ""))
  print(p)
  dev.off()
  return(p)
} 
...
DF <- matrix(NA,ncol = 4, nrow = length(B)*length(M)*length(mu)*length(E)) #The experimental matrix
i = 1
...
DF <- DF[which(DF[,1]!=0),]
colnames(DF) <- c("components","blocks","free components","extremeness")
...
t1 = Sys.time()
comparison_list1 <- foreach(l = 1:4,
                           .packages = c("combinat", "matrixcalc", "reshape2",
                                         "ggplot2")) %dopar% {
                                           comparison(DF[l,])
                                         }
comparison_list2 <- foreach(l = 5:8,
                            .packages = c("combinat", "matrixcalc", "reshape2",
                                          "ggplot2")) %dopar% {
                                            comparison(DF[l,])
                                          }
comparison_list3 <- foreach(l = 9:10,
                            .packages = c("combinat", "matrixcalc", "reshape2",
                                          "ggplot2")) %dopar% {
                                            comparison(DF[l,])
                                          }
comparison_list4 <- foreach(l = 11:21,
                            .packages = c("combinat", "matrixcalc", "reshape2",
                                          "ggplot2")) %dopar% {
                                            comparison(DF[l,])
                                          }
comparison_list5 <- foreach(l = c(23:24),
                            .packages = c("combinat", "matrixcalc", "reshape2",
                                          "ggplot2")) %dopar% {
                                            comparison(DF[l,])
                                          }
comparison_list6 <- foreach(l = 25:36,
                            .packages = c("combinat", "matrixcalc", "reshape2",
                                          "ggplot2")) %dopar% {
                                            comparison(DF[l,])
                                          }
